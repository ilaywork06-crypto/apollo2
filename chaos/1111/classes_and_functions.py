...
class HealthRecord:
    def __init__(self, patient_id, weight, height, age, notes):
...
class Location:
    def __init__(self, name, latitude, longitude, country_code):
        self.name = name
        self.latitude = latitude
        self.longitude = longitude
        self.country_code = country_code
    def distance_to(self, other):
        return distance_km(self.latitude, self.longitude, other.latitude, other.longitude)
    def formatted(self):
...
class PaginatedResponse:
    ...
    def has_prev(self):
        return self.page > 1
def sort_by_field(records, field, reverse):
...
def group_by(records, key_field):
    ...
    return result
class DataTable:
    def __init__(self, columns, rows, title):
